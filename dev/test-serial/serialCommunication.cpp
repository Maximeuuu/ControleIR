...
int main() {
    ...
    serialPort.open("COM5", std::ios::binary | std::ios::in | std::ios::out | std::ios::trunc );  // Modifier le chemin du port série si nécessaire
	...
	while( true )
	{
		serialPort.seekp( std::ios::end );
		serialPort.seekg( std::ios::end );
		serialPort.clear();
		serialPort.flush();
		serialPort.sync();
		...
		char* thefile = new char[ serialPort.tellg() ]{0};
        serialPort.seekg(0, std::ios::beg);
        serialPort.read(thefile, serialPort.tellg() );
        std::cout << thefile;

		std::cout << "2" << std::endl;

		if( std::getline(serialPort, receivedData) )
		{
			std::cout << "Données reçues : " << receivedData << std::endl;
		}
		else
		{
			std::cout << "Ligne non lue" << std::endl;
		}
